...
session = berserk.TokenSession(config["LICHESS_TOKEN_DEV"])
...
def get_move(agent, game_state, fen_string: str, turn: bool, book: bool = True) -> str:
    ...
    if book:
        with chess.polyglot.open_reader("Perfect2021.bin") as reader:
            try:
                return reader.weighted_choice(board).move.uci()
            except IndexError:
                print("Out of book")
