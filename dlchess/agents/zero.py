...
class ZeroAgent(Agent):
    def __init__(
        ...
        self.opt = (
            ...
            else RMSprop(learning_rate=0.001)
        )
    ...
    def set_noise(self, noise: float = 0.01):
        self.noise = noise

    def set_verbosity(self, verbose: bool):
        self.verbose = verbose

    def select_branch(self, node: ZeroTreeNode, _list: bool = False):
    ...
    def create_node(self, game_state, move=None, parent=None):
        ...
        if self.noise is not None:
            noise = np.random.dirichlet([self.noise] * priors.shape[0])
            priors = np.average([priors, noise], axis=0, weights=[0.5, 0.5])
